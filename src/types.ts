...
export type Dimension=CapabilityName;
...
export interface BenchmarkSummary{benchmarkVersion:string;scenarios:number;trials:number;passedTrials:number;successRate:number;averageDurationMs:number;capabilityScores:CapabilityScores;profile:string;adapter?:AdapterKind;cluster?:ClusterTopology}
export type ClusterTopology='single'|'dual'|'triple'|'quad'|'octa';
export interface BenchmarkConfig{model:string;adapter:AdapterKind;tier:Tier|'all';trials:number;baseUrl:string;apiKey?:string;timeoutMs:number;profile:string;concurrency:number;benchmarkVersion:string;output?:string;resume?:string;endpointPath:string;cluster?:ClusterTopology;withPerf?:boolean;category?:string;}
