...
export interface BenchmarkConfig{model:string;adapter:AdapterKind;tier:Tier|'all';trials:number;baseUrl:string;apiKey?:string;timeoutMs:number;profile:string;concurrency:number;benchmarkVersion:string;output?:string;resume?:string;endpointPath:string;cluster?:ClusterTopology;withPerf?:boolean;category?:string;source:ScenarioSource;numCtx?:number;keepAlive?:string;}
